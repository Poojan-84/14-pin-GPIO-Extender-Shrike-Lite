...
1. **Inconsistent/Random Reads:**
   - Check that FPGA synthesis completed without errors
   - Verify Place & Route succeeded (look for "FATAL ERROR" in build log)
   - Re-program the FPGA bitstream

2. **Duplicate Bytes on Read:**
   - Ensure delays between byte reads (`time.sleep_us(20)`)
   - Verify `byte_select` logic is included in FPGA code

3. **Synthesis Failures:**
   - Reduce design complexity
   - Check for proper synthesis attributes
   - Verify I/O planner mapping is correct

4. **Floating Pin Readings:**
   - Normal for unconnected input pins
   - Use external pull-up/pull-down resistors if needed
   - Or set pins as outputs to drive defined levels

---

## Future Enhancements

- [ ] Interrupt support on pin change
- [ ] PWM output capability
- [ ] Higher-speed SPI operation (tested up to 4 MHz)
- [ ] 16-bit or 32-bit expansion
